...
#'
#' The \code{eavesdropper_X} widget displays a vertical moving window of data
#' (see \code{eavesdropper} for a horizontal window). The Y-axis is time-lagged
...
#'
#' @section Usage:
#'    \preformatted{eavesdropper_X(eavesdrop, minimum = 1, maximum = 100,
...
#' @inheritParams eavesdropper_X
#' @import tcltk magrittr
#' @export
eavesdropper_X <- function(.switchboard, ...) {
  switchboard_engine(.switchboard,
                     constructor = eavesdropper_X_construct(.switchboard, ...),
                     updater = eavesdropper_X_update(.switchboard, ...), ...)
}

#' helper function that constructs canvas items of a eavesdropper_X widget
#' @keywords internal
eavesdropper_X_construct <- function(.switchboard,
                                     label = " ",
                                     size = 1,
                                     switch = FALSE,
                                     ...) {
...
#' helper function that updates the canvas items of a eavesdropper_X widget
#' @keywords internal
eavesdropper_X_update <- function(.switchboard,
                                  eavesdrop = NULL,
                                  minimum = " ",
                                  maximum = " ",
                                  size = 1,
  ...
  if(switchboard.env$totalIterations %% updates == 0) {
    ...
    if(as.numeric(tcltk::tclvalue(paste0(aCanvas, "Switch")))) {
      ...
      tcltk::.Tcl(paste("tcltk_eavesdropper_X_update", aCanvas, "movingPoint", size, theX, switchboard.env$rangeColors[floor(theX/size)]))
