...
#'
#' The \code{progress_ring_percent} widget displays an ring/circle progress bar
#' along with the percent complete.
#'
#' @section Usage:
#'    \preformatted{progress_ring_percent(eavesdrop, maximum = 100, caption = "",
...
#' @name progress_ring_percent
NULL


#' @inheritParams progress_ring_percent
#' @import tcltk magrittr
#' @export
progress_ring_percent <- function(.switchboard, ...) {
  switchboard_engine(.switchboard,
                     constructor = progress_ring_percent_construct(.switchboard, ...),
                     updater = progress_ring_percent_update(.switchboard, ...), ...)
}


#' helper function that constructs canvas items of a progress_ring_percent widget
#' @keywords internal
progress_ring_percent_construct <- function(.switchboard,
                                            label = " ",
                                            size = 1,
                                            ...) {
  ...
  tcltk::tkcreate(aCanvas, "text", 36 * size, 39 * size, text = "0", anchor = "center", font = paste(switchboard.env$font, floor(15 * size / 1.2)), fill = switchboard.env$mainColors[3], tags = "percent")
  tcltk::tkcreate(aCanvas, "text", 38 * size, 51 * size, text = label, anchor = "center", font = paste(switchboard.env$font, floor(9 * size / 1.2)), fill = switchboard.env$mainColors[3])
...
#' @keywords internal
progress_ring_percent_update <- function(.switchboard,
                                         eavesdrop,
                                         maximum = 100,
                                         size = 1,
                                         updates = 100,
  ...
  if(eavesdrop %% ceiling(maximum / updates) == 0) {
    ...
    testy <- tcltk::tkcreate(aCanvas, "oval", theX - 5 * size, theY - 5 * size, theX + 5 * size, theY + 5 * size, width = 0, fill = switchboard.env$mainColors[1], tags = "smallCircle")
